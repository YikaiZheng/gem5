...
traffic = [round(x * 0.01, 2) for x in range(1, 36)]
traffic2 = [round(x * 0.01, 2) for x in range(1, 26)]
traffic3 = [round(x * 0.01, 2) for x in range(1, 41)]
latency = []
latency2 = []
latency3 = []
reception_rate=[]
...
for current in traffic2:
    os.system(
        f"./build/NULL/gem5.opt configs/example/garnet_synth_traffic.py --network=garnet --num-cpus={cpus} --num-dirs={cpus} --topology=Ring --inj-vnet=0 --vcs-per-vnet=4 --routing-algorithm=4 --synthetic=uniform_random --sim-cycles=100000 --garnet-deadlock-threshold=100000 --injectionrate={current}"
    )
...
for current in traffic3:
    os.system(
        f"./build/NULL/gem5.opt configs/example/garnet_synth_traffic.py --network=garnet --num-cpus={cpus} --num-dirs={cpus} --topology=Mesh_XY --mesh-rows={int(cpus**0.5)} --inj-vnet=0 --vcs-per-vnet=4 --routing-algorithm=0 --synthetic=uniform_random --sim-cycles=100000 --garnet-deadlock-threshold=100000 --injectionrate={current}"
    )
    os.system(f"echo {current} >> network_stats3.txt")
    os.system(
        f'grep "average_packet_queueing_latency" m5out/stats.txt | sed "s/system.ruby.network.average_packet_queueing_latency\s*/average_packet_queueing_latency = /" >> network_stats3.txt'
...
plt.figure()
plt.plot(traffic, latency, marker="o", label="Spidergon")
plt.plot(traffic2, latency2, marker="+", label="Ring")
plt.plot(traffic3, latency3, marker="x", label="Mesh_XY")
plt.xlabel("Injection Rate")
plt.ylabel("Latency")
plt.title(
    "Latency-throughput curve for different topologies\n (4 VCs, Uniform Random, 64 nodes)"
)
plt.legend()
plt.savefig("1.png")

plt.figure()
plt.plot(traffic, hops, marker="o", label="Spidergon")
plt.plot(traffic2, hops2, marker="+", label="Ring")
plt.plot(traffic3, hops3, marker="x", label="Mesh_XY")
plt.xlabel("Injection Rate")
plt.ylabel("Hops")
plt.title(
    "Hops-throughput curve for different topologies\n (4 VCs, Uniform Random, 64 nodes)"
)
plt.legend()
plt.savefig("2.png")

plt.figure()
plt.plot(traffic, reception_rate, marker="o", label="Spidergon")
plt.plot(traffic2, reception_rate2, marker="+", label="Ring")
plt.plot(traffic3, reception_rate3, marker="x", label="Mesh_XY")
plt.xlabel("Injection Rate")
plt.ylabel("Reception Rate")
plt.title(
    "Reception Rate curve for different topologies\n (4 VCs, Uniform Random, 64 nodes)"
)
plt.legend()
plt.savefig("3.png")
